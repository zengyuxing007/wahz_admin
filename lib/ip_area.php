...
class ip_area {
	...
	public function __construct() {
		$this->func = 'data_full';
		$this->fp = @fopen(ROOT_PATH .'/lib/qqwry.dat', 'rb');

		/*
		if(@file_exists(ROOT_PATH .'/lib/mini_t.Dat')) {
			$this->func = 'data_mini';
			$this->fp = @fopen(ROOT_PATH .'/lib/mini.Dat', 'rb');
			$this->offset = unpack('Nlen', fread($this->fp, 4));
			$this->index  = fread($this->fp, $this->offset['len'] - 4);
		} elseif(@file_existes(ROOT_PATH .'/lib/qqwry.dat')) {
			$this->func = 'data_full';
			$this->fp = @fopen(ROOT_PATH .'/lib/qqwry.dat', 'rb');
		}
		*/
	...
	*/
	...
	function getCityAddress($ip) {
		$city = $this->data_full($ip);
		$arr = explode(' ', $city);
		return $arr[0];
	}
